...
- Requirements: Node 22.22.0, pnpm 11.2.2
- Recommended: use mise to pin tools and run tasks

```sh
# Setup toolchain + install deps
mise install                 # installs Node 22.22.0 and pnpm 11.2.2
pnpm install
...
- Config: `tsconfig*.json`, `tests/tsconfig.json`, `tests-d/tsconfig.json`, `tsup.config.ts`, `jest.config.cjs`, `.oxlintrc.json`, `mise.toml`
...
- Oxlint config warnings: run `pnpm oxlint --print-config` to inspect the resolved config
